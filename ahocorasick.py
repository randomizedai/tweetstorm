...
def unique_matches(matches):
    ...
    for i,match in enumerate(matches):
        for x in range(match['beginpos'],match['endpos']):
            if (x not in dictv) or (x in dictv and (match['endpos'] - match['beginpos']) > dictv[x][1]):
                dictv[x] = (i,(match['endpos'] - match['beginpos']))

    for x in list(set([ y[0] for y in dictv.values()])):
        ans.append(x)
     
    return ans      
     
if (__name__ == "__main__"):
    list_keywords = ["can't","climate change", "change of climate", "kyoto protocol", "kyoto protocol a1", "el nino"]
    tagger = getTagger(list_keywords)
    l = tagger.tag("kyoto protocol a1")
    print unique_matches(l)
